...
describe('Sedra', () => {
  const mapper = new Mapper(fromSedraWriting, toCalWriting);
  const { removeDotting } = mapper;

  describe('removeDotting', () => {
    it('General case usage, with one-to-one mapping', () => {
      const word = removeDotting('LADNH');
      const vocalised = removeDotting('LeAD,NeH');
      const wordExpected = word;
      strictEqual(word, wordExpected, 'removeDotting_generic consonant');
      strictEqual(vocalised, wordExpected, 'removeDotting_generic vocalised');
    });
    it('General case usage, un-mapped symbols', () => {
      const word = removeDotting('<LADNH>');
      const vocalised = removeDotting('<LeAD,NeH>');
      const wordExpected = word;
      strictEqual(word, wordExpected, 'removeDotting consonant with unmapped');
      strictEqual(
        vocalised,
        wordExpected,
    ...
    it('Another one-to-one mapping', () => {
      const word = removeDotting('ABHOH;');
      const vocalised = removeDotting('AaB,oHaOH_;');
      const wordExpected = word;
      strictEqual(word, wordExpected, 'removeDotting_wu consonant');
      strictEqual(vocalised, wordExpected, 'removeDotting_wu vocalised');
    });
    it('Word with (i;) => (yi) mapping', () => {
      const word = removeDotting('D;L;DOTH');
      const vocalised = removeDotting("D'i;Li;D,uOT,eH");
      const wordExpected = word;
      strictEqual(word, wordExpected, 'removeDotting_yi consonant');
      strictEqual(vocalised, wordExpected, 'removeDotting_yi vocalised');
    });
    it('Word with (uO) => (wu) mapping', () => {
    ...
    it('Word with (oO) => (wO) mapping', () => {
      const word = removeDotting('BTSLON;XA');
      const vocalised = removeDotting("B'T,eSaLoONi;XiA");
      const wordExpected = word;
      strictEqual(word, wordExpected, 'removeDotting_wO consonant');
      strictEqual(vocalised, wordExpected, 'removeDotting_wO vocalised');
    });
    it('Blank word returns blank', () => {
    ...
    it('Null word returns null', () => {
      ...
      const wordExpected = null;
      strictEqual(word, wordExpected, 'removeDotting_null');
    });
    it('Undefined word returns undefined', () => {
      const word = removeDotting(undefined);
      const wordExpected = undefined;
      strictEqual(word, wordExpected, 'removeDotting_undefined');
    });
    it('0 number as word returns 0', () => {
      const word = removeDotting(0);
      const wordExpected = 0;
      strictEqual(word, wordExpected, 'removeDotting_zero');
    });
  });
});
